label playerClass:
    python:
        ...
        class Player:
            #CODEMOD: New player instances use StashInventory rather than Inventory
            def __init__(self, name="", gender="male", skillList=[], perks=[], stats=Stats(), inventory=StashInventory(), statPoints = 5,
            ...
            def _resetPlayerAtStart(self, theFetishList):
                ...
                #CODEMOD: Replace Inventory with StashInventory
                self.inventory=StashInventory()
                self.combatStance=[CombatStance()]
